...
sub parse_adv {
    ...
    # дата размещения
    my $date_str = trim($dom->find('div[class~="productPage__createDate"] span')->first->text);
    ...
    my @photos;
    $dom->find('div[class~="js-lineProductGallery"] img')->each ( sub {
    ...
    if (my $user_node = $dom->find('div[class~="productPage__inlineWrapper"]')->first) {
        ...
        if ($seller =~ /(компан)|(агенст)|(недвиж)/) {
